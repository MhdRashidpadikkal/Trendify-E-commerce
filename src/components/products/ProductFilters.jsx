...
const ProductFilters = () => {
    ...
    console.log("updated prices in context at filter hook", state.priceRangeMax + "min value is", state.priceRangeMin);
    ...
    return (
        ...
        <div className={`md:w-[20%] md:border-2 bg-white border-r-2 rounded-md ${state.isFilterBar ? "block" : "hidden"} md:block px-5 absolute left-0 top-0 md:relative py-3 ${!state.showAllCategories ? "h-[750px]" : "h-[1010px]"} `}>
            <CircleX onClick={() => dispatch({type:"HIDE_FILTER_BAR"})} className='absolute right-2 cursor-pointer md:hidden' />
